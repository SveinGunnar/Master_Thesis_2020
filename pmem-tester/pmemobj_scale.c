...
{	
	...
	for(i=0;i<10;i++){
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			drm_write_array1[j] = scalar*drm_read_array1[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			D_RW(nvm_write_array1)[j] = scalar*drm_read_array2[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			drm_write_array2[j] = scalar*D_RO(nvm_read_array1)[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			D_RW(nvm_write_array2)[j] = scalar*D_RO(nvm_read_array2)[j];
		}
		//Time stop.
