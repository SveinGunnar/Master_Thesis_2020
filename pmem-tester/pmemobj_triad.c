...
{	
	...
	pop = pmemobj_create(path, LAYOUT_NAME, 10737418240, 0666);
        ...
        int i,j,k;
	double scalar = 3.0;
	double bytes = 3 * sizeof(double) * ARRAY_LENGTH;
	...
	double *drm_read_array3 = (double*)malloc(ARRAY_LENGTH*sizeof(double));
        double *drm_read_array4 = (double*)malloc(ARRAY_LENGTH*sizeof(double));
	...
	TOID(double) nvm_read_array3;
        TOID(double) nvm_read_array4;
	...
	POBJ_ALLOC(pop, &nvm_read_array3, double, sizeof(double) * ARRAY_LENGTH, NULL, NULL);
        POBJ_ALLOC(pop, &nvm_read_array4, double, sizeof(double) * ARRAY_LENGTH, NULL, NULL);
	...
	double e=1.40, f=3.379, g=2.423, h=7.5499;
		...
		drm_read_array3[i] = ((double)rand()/(double)(RAND_MAX)) * e;
                drm_read_array4[i] = ((double)rand()/(double)(RAND_MAX)) * f;
                D_RW(nvm_read_array1)[i] = ((double)rand()/(double)(RAND_MAX)) * c;
		D_RW(nvm_read_array2)[i] = ((double)rand()/(double)(RAND_MAX)) * d;
		D_RW(nvm_read_array3)[i] = ((double)rand()/(double)(RAND_MAX)) * g;
                D_RW(nvm_read_array4)[i] = ((double)rand()/(double)(RAND_MAX)) * h;
                //printf("%lf\n", nvm_float_array[i] );
        }
	...
	for(i=0;i<10;i++){
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			drm_write_array1[j] = drm_read_array1[j]+scalar*drm_read_array2[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			D_RW(nvm_write_array1)[j] = drm_read_array3[j]+scalar*drm_read_array4[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			drm_write_array2[j] = D_RO(nvm_read_array1)[j]+scalar*D_RO(nvm_read_array2)[j];
		}
		//Time stop.
		...
		for(j=0;j<ARRAY_LENGTH;j++){
			D_RW(nvm_write_array2)[j] = D_RO(nvm_read_array3)[j]+scalar*D_RO(nvm_read_array4)[j];
		}
		//Time stop.
	...
	free(drm_read_array3);
        free(drm_read_array4);
	...
	POBJ_FREE(&nvm_read_array3);
        POBJ_FREE(&nvm_read_array4);
